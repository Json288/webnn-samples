...
/**
 * ResNet-18 image classification via ONNX model (resnet18.onnx).
 * Uses ONNX Runtime Web with WebNN execution provider.
 * Place your resnet18.onnx in image_classification/models/resnet18.onnx.
 */
export class ResNet18Onnx {
  constructor() {
    this.session_ = null;
  ...
  async load(contextOptions) {
    ...
    const modelUrl = './models/resnet18.onnx';
    ...
    if (!inputs.length || !outputs.length) {
      throw new Error('ResNet18 ONNX: model must have at least one input and output');
    }
  ...
  async compute(inputBuffer) {
    ...
    if (!output || !output.data) {
      throw new Error('ResNet18 ONNX: no output from session.run');
    }
